...
def make_env(seed=None):
    def _init():
        env = gym.make("AssaultNoFrameskip-v4", render_mode=None,)
...
def train_dqn(hyperparams, policy_type):
    ...
    model = DQN(
        ...
        env=env,
        verbose=1,
        tensorboard_log=f"logs/{policy_type.lower()}_assault",
    ...
    # Set up callbacks
    train_logger = TrainingLogger(f"results/{policy_type.lower()}_metrics.csv")
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=f"models/best_{policy_type.lower()}",
        log_path=f"logs/{policy_type.lower()}_eval",
        eval_freq=10000,
        deterministic=True,
        render=False
    )
    ...
    model.save(f"models/dqn_{policy_type.lower()}_final")
